...
export class ViewMyTweetsComponent implements OnInit {
  ...
  constructor(private activatedRoute:ActivatedRoute,private userService:UserService,private tweetService:TweetService,private router:Router) { }
  ...
  deleteTweet(tweetId)
  {
    this.tweetService.DeleteTweet(tweetId).subscribe(res=>{
      if(res)
      {
        alert("Tweet Deleted");
      }
    })
    this.router.navigateByUrl("/view-my-tweets/"+sessionStorage.getItem("username"));
  }
}
